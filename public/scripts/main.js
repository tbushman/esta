...
new Vue({
	...
	data: function data(){
		return {
			buf: null,
			availablelayers: this.ifNullThenArr(availablelayers),
			...
			j: 0,
			m: 0,
			signable: this.parseBool(signable),
			unsigned: this.parseBool(unsigned),
			avail: this.parseBool(avail),
			type: this.parseObj(type),
			pu: this.parsePu(pu),
			loggedin: (this.parseObj(session) !== '' && this.parseObj(session.loggedin) !== '' ? this.parseObj(session.loggedin) : null ),
			menu: this.parseObj(menu),
			dat: this.parseObj(dat),
			doc: this.parseObj(doc),
			layers: this.parseObj(layers),
			position: (session.position ? {lat: 40, lng: -111.89, zoom: 6}/*null*/ : this.parseObj(session.position)),
			edit: (this.parseObj(doc) === '' ? null : this.parseObj(doc.index)),
			chindexes: [],
			chindex: 0,
			dindexes: [],
			dindex: (this.parseObj(doc) === '' ? 0 : this.parseObj(doc.index)),
			cursor: null,
			input: '',
			fixedPug: '',
			map: '',
			...
			dragind: null,
			latlngs: null,
			//- gp: (this.parseObj(#{session.importgdrive}) !== '' ? this.parseObj(gp) : null),
			sliderIndex: 1,
			sliderInterval: '',
			sliderTimeout1: '',
			sliderTimeout2: '',
			str: this.parseObj(str),
			ts: this.ifNullThenArr(ts),
			can: [],
			lyr: {},
			geo: [],
			tis: [
			...
			//- 	{code: '970', name:'school district (unified)'}],
				...
				//(!this.parseObj(dat)}).length ? [[]] : Array(this.parseObj(!{JSON.stringify(dat).length).fill([]))
